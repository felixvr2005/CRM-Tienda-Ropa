...
const POST = async ({ request }) => {
  try {
    ...
    const { data: returnRequest, error: fetchError } = await supabaseAdmin.from("return_requests").select("*").eq("id", returnId).single();
    ...
    const { data: customer, error: customerError } = await supabaseAdmin.from("customers").select("email, first_name, last_name").eq("id", returnRequest.customer_id).single();
    if (customerError) {
      console.error("Error fetching customer:", customerError);
    }
    console.log("[Confirm Return API] Cliente obtenido:", customer?.email);
    console.log("[Confirm Return API] Obteniendo pedido...");
    const { data: order, error: orderError } = await supabaseAdmin.from("orders").select("order_number").eq("id", returnRequest.order_id).single();
    if (orderError) {
    ...
    const { error: updateError } = await supabaseAdmin.from("return_requests").update(updateData).eq("id", returnId);
    if (updateError) {
