...
const getReadableDateRange = (range, startDate, endDate) => {
  const today = /* @__PURE__ */ new Date();
...
const fetchOrdersData = async (from, to) => {
  try {
...
const fetchShipmentsData = async (from, to) => {
  try {
    const { data, error } = await supabase.from("shipments").select(`
                ...
                shipped_at,
                orders (order_number)
            `).gte("shipped_at", from).lte("shipped_at", to).order("shipped_at", { ascending: false });
    if (error) throw error;
    return data || [];
  } catch (error) {
...
const fetchAlerts = async (from, to) => {
  const alerts = [];
  try {
    const { data: paymentErrors } = await supabase.from("orders").select("id, order_number, status, created_at").eq("status", "payment_failed").gte("created_at", from).lte("created_at", to);
    ...
    const { data: lowStock } = await supabase.from("products").select("id, name, sku, stock_quantity").lt("stock_quantity", 10);
    ...
    const { data: incompleteOrders } = await supabase.from("orders").select("id, order_number, status").in("status", ["pending", "processing"]).gte("created_at", from).lte("created_at", to);
...
const calculateFinancialMetrics = (orders) => {
  ...
  const commissions = grossRevenue * 0.05;
...
const getTopProducts = async (orders) => {
  ...
  })).sort((a, b) => b.product_quantity - a.product_quantity).slice(0, 5);
};
const generateAdminEmailData = async (dateRange, startDate, endDate) => {
  ...
  return {
    ...
    report_date: (/* @__PURE__ */ new Date()).toLocaleDateString("es-ES", {
      weekday: "long",
      year: "numeric",
      month: "long",
      day: "numeric"
    }),
    generated_at: (/* @__PURE__ */ new Date()).toLocaleString("es-ES"),
    ...
    gross_revenue: financials.grossRevenue.toFixed(2),
    refunds: financials.refunds.toFixed(2),
    shipping_costs: financials.shippingCosts.toFixed(2),
    discounts_total: financials.discounts.toFixed(2),
    commissions: financials.commissions.toFixed(2),
    net_profit: financials.netProfit.toFixed(2),
    ...
    average_order_value: (totalRevenue / orders.length || 0).toFixed(2),
    conversion_rate: (orders.length / 1e3 * 100).toFixed(2),
    // Estimado
...
const exportReportData = async (dateRange, format = "csv", startDate, endDate) => {
  const { from, to } = getDateRange(dateRange, startDate, endDate);
  const orders = await fetchOrdersData(from, to);
  if (format === "json") {
...
export { exportReportData as e, generateAdminEmailData as g };
